...
for i in $(seq 1 ${orgs}); do
  ...
  tower-cli group create -n "localhost_group" -d "Only localhost in here" -i "localhost_only${i}" -u admin -p $TOWERPW
  ...
  group_id=$(tower-cli group list --name localhost_group -a -u admin -p $TOWERPW | sed -e 's/^[[:space:]]*//' | awk -F" " -v inv_id=${inv_id} '{if($3==inv_id) print $1; }')
